"""
Specific routines for defining and designing tr_7500 beams.
...
class Beam(BeamBase):
    """Beam object for design class: tr_7500."""
    ...
    def compute_required_transverse_reinforcement(self) -> None:
        ...
        # Oblique cracking strength of concrete
        Vcr = 0.65 * self.fctd * self.b * (self.h - self.cover)

        # Transverse reinforcement computation, Section 8.3 in TBEC-1984
        Ash_sbh = np.zeros(len(Vd))
        mask = Vd <= Vcr
        Ash_sbh[mask] = self.rhoh_min * self.b
        Ash_sbh[~mask] = Vd[~mask] / (self.fsyd * (self.h - self.cover))
