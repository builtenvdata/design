"""
Specific routines for defining and designing tr_0007 buildings.
...
since TBEC-1998 enforces to not consider roof level columns in terms
of capacity design principles.
2- The method named "_set_column_capacity_design_shear_forces"
overwritten to calculate column capacity shear forces using beam
capacity moments, except for ground floor columns, which are
treated as in TBEC-1998.
3- The method named "_set_beam_capacity_design_shear_forces"
overwritten since the rules are different in TBEC-1998 compared
to EN 1998-1:2004.
4- The method named "_predesign" overwritten to add slab type as
an input for predesign_section_dimension method of beams.
"""

# Imports from installed packages
from typing import List, Type, Tuple

# Imports from the design class (tr_0007) library
...
class Building(BuildingBase):
    """Building object for design class: tr_0007."""
    ...
    def _change_materials(self) -> None:
        ...
        Notes
        -----
        It is overwritten for tr_0007 design class with following changes:
    ...
    def _set_column_capacity_design_shear_forces(self) -> None:
        ...
        for column in self.columns:
            ...
            if (
                ...
                # combos (1,3) are belongs to seismic combos in x and y
                # directions)
                ratio_x_top = abs(
                    joint_j.bottom_column.design_forces[3].Mx9) / (
                    abs(joint_j.bottom_column.design_forces[3].Mx9) +
                    abs(joint_j.top_column.design_forces[3].Mx1)
                )
                ratio_y_top = abs(
                    joint_j.bottom_column.design_forces[1].My9) / (
                        abs(joint_j.bottom_column.design_forces[1].My9) +
                        abs(joint_j.top_column.design_forces[1].My1)
                )
                ratio_x_bot = abs(
                    joint_i.top_column.design_forces[3].Mx1) / (
                    abs(joint_i.top_column.design_forces[3].Mx1) +
                    abs(joint_i.bottom_column.design_forces[3].Mx9)
                )
                ratio_y_bot = abs(
                    joint_i.top_column.design_forces[1].My1) / (
                    abs(joint_i.top_column.design_forces[1].My1) +
                    abs(joint_i.bottom_column.design_forces[1].My9)
                )

                # Column plastic moments to be used for shear capacity forces,
                # Reference: Section-7.3.7 in TBEC-1998
    ...
    def _set_beam_capacity_design_shear_forces(self) -> None:
        ...
        for beam in self.beams:
            ...
            Mpi_pos = gamma_rd * beam.pos_Mrd[0]
            Mpi_neg = gamma_rd * beam.neg_Mrd[0]
            Mpj_pos = gamma_rd * beam.pos_Mrd[-1]
            Mpj_neg = gamma_rd * beam.neg_Mrd[-1]
