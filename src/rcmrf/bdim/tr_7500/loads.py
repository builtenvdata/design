"""
Pydantic models for defining loads in tr_7500 buildings.
"""
...
from ..baselib.loads import CombinationBase, DataFormatBase, LoadsBase


class Variable(VariableBase):
    """
    Model representing live loads (Q) for design class tr_7500.
...
class Permanent(PermanentBase):
    """
    Model representing superimposed dead loads (G) for design class tr_7500.
...
class Combination(CombinationBase):
    """
    Model representing a load combination for design class tr_7500.
...
class DataFormat(DataFormatBase):
    """
    Model representing the format of loads data for design class tr_7500.
...
class Loads(LoadsBase):
    ...
    """Path to the file containing loads data."""
    _data_model: Type[DataFormat]

    def __init__(self) -> None:
        """Initializes Loads object.
        """
        self._data_model = DataFormat
        super().__init__()
