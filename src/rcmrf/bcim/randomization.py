...
class Sampler:
    ...
    def set_slab_type(
        ...
        (minimum span length <= `upper_lim_for_min_ss_span_length`).
        3. Otherwise, if the ratio of maximum to minimum span lengths is
        greater or equal to `max_span_ratio_for_ss2`, slab type is
        ...
        >>> upper_lim_for_min_ss_span_length = 6.0  # in meters
        ...
        for i in range(self.sample_size):
            if min_span_length[i] > upper_lim_for_min_ss_span_length:
                slab_type[i] = 3
            else:
                if max_span_ratio[i] >= upper_lim_for_max_ss2_span_ratio:
