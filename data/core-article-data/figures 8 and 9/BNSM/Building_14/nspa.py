...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.10363663356087667, 0, 0, 0, 0, 0)
    ops.load(92000, 0.22064286538767308, 0, 0, 0, 0, 0)
    ops.load(93000, 0.34983115751950905, 0, 0, 0, 0, 0)
    ops.load(94000, 0.32588934353194116, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.1100318166420205, 0, 0, 0, 0)
    ops.load(92000, 0, 0.23064301393750788, 0, 0, 0, 0)
    ops.load(93000, 0, 0.3504235189056675, 0, 0, 0, 0)
    ops.load(94000, 0, 0.30890165051480406, 0, 0, 0, 0)
