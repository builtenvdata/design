"""
Specific routines for defining and designing tr_0018_cdm columns.
...
class Column(ColumnBase):
    """Column object for design class: tr_0018_cdm."""
    ...
    def compute_required_transverse_reinforcement(self) -> None:
        ...
        if Ve_x <= Vcr_x:
            Ashx_sh = self.rhoh_min * dy
        else:
            Vw = Ve_x - Vc_x
            Ashx_sh = Vw / (self.fsyd * dx)

        if Ve_y <= Vcr_y:
            Ashy_sh = self.rhoh_min * dx
        else:
            Vw = Ve_y - Vc_y
        ...
        self.Ashx_sbh_req = Ashx_sh
        self.Ashy_sbh_req = Ashy_sh
