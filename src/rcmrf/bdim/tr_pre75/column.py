...
class Column(ColumnBase):
    """Column object for design class: tr_pre75.
    """
    steel: Steel
    """Steel material."""
    concrete: Concrete
    ...
    def apply_section_compatibility(self) -> None:
        ...
        if self.section == 1:  # Square section
            ...
            self.bx = max(self.bx, self.by)
            self.by = max(self.bx, self.by)
        elif self.section == 2:  # Rectangular section
            pass
    ...
    def compute_required_transverse_reinforcement(self) -> None:
        ...
        for force in self.design_forces:
            ...
            # Aslx = 0.5*rhol_x*self.Ag
            # Asly = 0.5*rhol_y*self.Ag
            # TODO: Originally Aslx and Asly was computed for each load combo
            # Does this makes sense? now I am using the available Aslx and Asly
            ...
            zx = 0.9 * dx
            zy = 0.9 * dy
            # TODO: Shouldn't we use the finalised long. reinf. for the
            # calculation of shear reinforcement?
            # (Asl_req after completing the previous loop)
            # Design forces
            Vy1 = abs(force.Vy1)
            Vx1 = abs(force.Vx1)
            Vy9 = abs(force.Vy9)
