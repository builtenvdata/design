"""
Specific routines for defining and designing tr_post18_cdm columns.
...
class Column(ColumnBase):
    """Column object for design class: tr_post18_cdm."""
    ...
    def envelope_forces_overstrength_adjusted(self) -> ColumnEnvelopeForces:
        ...
        attributes = ["N1", "Mx1", "Vy1", "My1", "Vx1", "N9", "Mx9",
                      "Vy9", "My9", "Vx9"]

        # Find minimum and maximum for each attribute
        min_values = [
            min(getattr(force, attr) for force in
                self.design_forces_overstrength_adjusted)
            for attr in attributes]
        max_values = [
            max(getattr(force, attr) for force in
                self.design_forces_overstrength_adjusted)
            for attr in attributes]
    ...
    def verify_section_adequacy(self) -> None:
        ...
        # Maximum shear force
        max_Vx = max(self.envelope_forces_overstrength_adjusted.Vx1,
                     self.envelope_forces_overstrength_adjusted.Vx9)
        max_Vy = max(self.envelope_forces_overstrength_adjusted.Vy1,
