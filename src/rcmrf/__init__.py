...
def generate(
    inputs: Dict[Literal['bcim', 'fim'], Dict], outdir: str | Path = None
        ) -> Tuple[BCIM, List[BuildingBase | None], List[FIM | None]]:
    """Generates BCIM, BDIM and FIM data by following complete workflow
    in bed framework for rcmrf systems.

    Parameters
    ----------
    inputs : Dict[Literal['bcim', 'fim'], Dict]
    ...
    # Get fim settings
    fim_inputs = inputs.get('fim')
    lang = 'py'
    if fim_inputs is None:
        fim_inputs = {}
    ...
    for i, taxonomy in enumerate(bcim.taxonomy):
        ...
        if bdim_.ok:  # If the design is ok, continue with the model
            ...
            # Initialize FIM
            fim_ = FIM(bdim_, **fim_inputs)
            if lang == 'py':
                # Export numerical models for OpenSeesPy
                fim_.to_py(fim_path / f'Building_{i+1}')
            elif lang == 'tcl':
                # Export numerical models for OpenSeesTcl
                fim_.to_tcl(fim_path / f'Building_{i+1}')
            # Plot the model and save
            fim_.plot_model(directory=fim_path / f'Building_{i+1}', show=False)
            # Add to the BDIM and FIM databases
            bdim.append(bdim_)
            fim.append(fim_)
        else:
            bdim.append(None)
