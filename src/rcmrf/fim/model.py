...
class FIM:
    """Fragility Information Model (FIM) class which can be used to define and
    run numerical models in OpenSees.
    ...
    def __get_nspa_loading_parameters(
        ...
        heights = []
        modal_disps = []
        num_modes = len(self.basic_masses)
    ...
    def __get_run_py(self) -> List[str]:
        ...
        """
        content = [
            "# Import different analysis methods",
            "from modal import do_modal",
            "from nspa import do_nspa_x, do_nspa_y",
            "",
            "",
            "# Perform modal analysis",
            "results = do_modal()",
    ...
    def plot_model(
        ...
        if directory:
            filename = str(Path(directory) / 'model.html')
        else:
            filename = None
        # Plot the model
